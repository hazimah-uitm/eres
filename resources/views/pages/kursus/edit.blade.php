    ...
    <div class="card-body">
        <form method="POST" action="{{ $save_route }}">
            ...
            <div class="mb-3">
                ...
                <select class="form-select {{ $errors->has('program_id') ? 'is-invalid' : '' }}" id="program_id" name="program_id">
                    @foreach ($programList as $program)
                    <option value="{{ $program->id }}"
                        {{ old('program_id') == $program->id || ($kursus->program_id ?? '') == $program->id ? 'selected' : '' }}>
                        {{ $program->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control {{ $errors->has('kod') ? 'is-invalid' : '' }}" id="kod"
                    name="kod" value="{{ old('kod') ?? ($kursus->kod ?? '') }}">
                @if ($errors->has('kod'))
                <div class="invalid-feedback">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" id="name"
                    name="name" value="{{ old('name') ?? ($kursus->name ?? '') }}">
                @if ($errors->has('name'))
                <div class="invalid-feedback">
            ...
            <div class="mb-3">
                ...
                <div class="form-check">
                    <input type="radio" id="aktif" name="publish_status" value="1"
                        {{ ($kursus->publish_status ?? '') == 'Aktif' ? 'checked' : '' }}>
                    <label class="form-check-label" for="aktif">Aktif</label>
                </div>
                <div class="form-check">
                    <input type="radio" id="tidak_aktif" name="publish_status" value="0"
                        {{ ($kursus->publish_status ?? '') == 'Tidak Aktif' ? 'checked' : '' }}>
