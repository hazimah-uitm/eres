...
<div class="card">
    <div class="card-body">
        ...
        <form method="POST" action="{{ $save_route }}" enctype="multipart/form-data">
            ...
            {{-- Maklumat Pengguna (pemilik rekod) --}}
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control"
                        value="{{ optional($user->ptj)->name ?? ($user->ptj_id ?? '-') }}" readonly>
                </div>
            </div>
                ...
                <select class="tom-select {{ $errors->has('program_id') ? 'is-invalid' : '' }}"
                    ...
                    @foreach ($programList as $program)
                        <option value="{{ $program->id }}"
                            {{ old('program_id') == $program->id || ($rekod->program_id ?? '') == $program->id ? 'selected' : '' }}>
                ...
                <select class="tom-select {{ $errors->has('kursus_id') ? 'is-invalid' : '' }}"
                    ...
                    @foreach ($kursusList as $kursus)
                        <option value="{{ $kursus->id }}"
                                data-program-id="{{ $kursus->program_id }}"
                                {{ old('kursus_id') == $kursus->id || ($rekod->kursus_id ?? '') == $kursus->id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control {{ $errors->has('kumpulan') ? 'is-invalid' : '' }}"
                       id="kumpulan" name="kumpulan"
                       value="{{ old('kumpulan') ?? ($rekod->kumpulan ?? '') }}" required>
            ...
            {{-- Fail PDF --}}
            <div class="mb-3">
                <label for="file_pdf" class="form-label">Fail PDF (Jika ingin gantikan)</label>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function resetKursus(keepSelected = false) {
        ...
        kursusTS.addOption({ value: '', text: 'Pilih Kursus' });
        kursusTS.refreshOptions(false);
    }

    function filterKursusByProgram(programId) {
        const currentSelected = kursusTS.getValue();
        ...
        allKursusOptions.forEach(o => {
            ...
            if (String(o.programId) === String(programId)) {
                kursusTS.addOption({ value: o.value, text: o.text });
            }
        });

        kursusTS.refreshOptions(false);

        // kalau pilihan kursus masih valid bawah program baru, kekalkan
        if (currentSelected) {
            const isStillValid = allKursusOptions.some(o =>
                String(o.value) === String(currentSelected) && String(o.programId) === String(programId)
            );
            if (isStillValid) {
                kursusTS.setValue(currentSelected, true);
            } else {
                kursusTS.clear(true);
            }
        }
    ...
    // initial: ikut program selected (rekod/old)
    const initialProgramId = programTS.getValue();
    if (initialProgramId) {
        filterKursusByProgram(initialProgramId);

        const oldKursusId = "{{ old('kursus_id') ?? ($rekod->kursus_id ?? '') }}";
        if (oldKursusId) {
